...
And the old graph again, just for ease of comparison:
```{r ex4_oldGraph-silent, echo=F}
...
### A comparison of the two graphs:
 ...
 - I didn't have to manually create a legend
 - The use of `geom_point( alpha = 0.3 )` allowed points to not overshadow eachother as much
 
### Things I did differently in the new graph:
 - I removed the redundant "(years)" part of the y-axis label
 - I improved the y-axis label by putting the units GDP is measured in
 - The x-axis tick marks are labelled with dollar signs
 
### Things I thought about doing differently but chose not to:
 - I considered putting a log x-axis with `scale_x_log10()`. It would declutter points and spread them out more, but I decided not to as the point I was originally trying to address was whether there is a certain GDP level that life expectancy gains are muted at. The linear x-axis shows that approach of the asymptote better, and allows easier reading of the x-axis values for the user to intepret.
 - I could also turn off the colour by continents, and keep all in one colour. This might distract less from the overall message in the graph and keep things more concise, also allowing me to do away with the legend. Ultimately, I thought there might still be interesting trends for the user to see if they wanted to look at certain continent trends within the overall picture.
