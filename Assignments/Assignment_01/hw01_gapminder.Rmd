...
library(gapminder)
library(plyr) #trusting that the TA will have this installed already
data("gapminder")
...
While Oceania is the richest continent on average, Kuwait, which is on the Asian continent, is the richest country overall. In the graph below, all countries with average income values greater than $50000 have been labelled. These include Kuwait, Norway, Singapore, and the United States.   
```{r boxplot, echo=FALSE}

{ 
  par(mar=c(5,6,1,1)) #change margins to fit continent labels
  
  boxplot(gm$gdpPercap~gm$continent, #make boxplot
        xlab="",
        ylab = "",
        yaxt="n",
        las=3,
...
You can see general groupings of GDP and life expectancy by continent in the graph below. Also, there seems to be an asymptotic relationship between life expectancy and GDP. Mean life expectancy doesn't increase much beyond 80 years, no matter how rich the country.   
```{r lifeExp_GDP, echo=FALSE}

leg.labels <- levels(as.factor(gm$continent)) #need labels for legend
